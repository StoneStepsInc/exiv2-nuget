...
options:
  ...
  * `EXIV2_ENABLE_PNG=ON`
  * `EXIV2_ENABLE_FILESYSTEM_ACCESS=ON`
...
Uncomment `goto :EOF` in `make-package.bat` after CMake generates
...
around that bug, rename `packages.config` to `packages.exiv2lib.config`.

    move build\src\packages.config build\src\packages.exiv2lib.config
...
For consistency, rename the newly-created `build\src\packages.config` to
...
is created for a single project and can remain as `package.config`.
    ...
    move build\src\packages.config build\src\packages.exiv2lib_int.config
...
Compare `exiv2lib.vcxproj`, `exiv2lib_int.vcxproj` and `exiv2-xmp.vcxproj`
in a diff tool and remove empty changes, such as XML blocks being expanded
or collapsed and revert the XML block containing this change because this
file no longer exists. Given that these project files are short-lived,
there are no side effects caused by this removal.
...
If your CMake version is lower than 3.30.0, open `build\exiv2.sln`
...
way using the base installation directory saved earlier.
    ...
    devops\create-vs-nuget-patches exiv2-0.28.0_base exiv2-0.28.0
